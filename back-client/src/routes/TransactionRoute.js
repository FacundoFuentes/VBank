...
transaction.post('/test', async(req,res) => {
 ...
 try {     
     ...
     if(!userFrom || !userTo) return res.status(400).json({status: 'failed', error: 'User not found or a meteorite landed on your house (or in the data center)'})
 ...
 if(accountFrom.balance - amount < 0) return res.status(400).json({status: 'failed', error: 'User dont have enough money'})
